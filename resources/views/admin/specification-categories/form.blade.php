...
<main class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center mb-6">
        <div><h1 class="h3 mb-1">{{ $category->exists ? 'Edit category' : 'Add category' }}</h1><p class="text-body-secondary mb-0">Create a logical group for specification definitions.</p></div>
        <a class="btn btn-outline-secondary" href="{{ route('admin.specification-categories.index') }}">Back to categories</a>
    </div>
    <x-auth-validation-errors class="mb-4" :errors="$errors" />
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ $category->exists ? route('admin.specification-categories.update', $category) : route('admin.specification-categories.store') }}" class="row g-4">
                @csrf
                ...
                <div class="col-12 d-flex gap-2"><button class="btn btn-primary" type="submit">{{ $category->exists ? 'Save changes' : 'Create category' }}</button><a class="btn btn-outline-secondary" href="{{ route('admin.specification-categories.index') }}">Cancel</a></div>
            </form>
        </div>
    </div>
</main>
